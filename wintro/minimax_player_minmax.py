...
class MinimaxPlayer(Player):
	...
	def minimax_search_utility(self, board: GameBoard, plies) -> float:
		if plies == 0:
			print("the heuristic is", heuristic(board))
			...
			for i in board.get_legal_moves():
				...
				if new_board != None:
					value = max(value, self.minimax_search_utility(new_board, plies-1))
			return value
			...
			for i in board.get_legal_moves():
				new_board = board.make_move(i)
				if new_board != None:
					value = min(value, self.minimax_search_utility(new_board, plies-1))
			return value
	...
	def choose_move(self, board: GameBoard) -> Optional[Location]:
		...
		if board.get_active_player() == MAX_PLAYER:
			...
			for i in board.get_legal_moves():
				new_board = board.make_move(i)
				if new_board != None:
					recursive_result = self.minimax_search_utility(new_board, self.plies) 
					if recursive_result > value:
